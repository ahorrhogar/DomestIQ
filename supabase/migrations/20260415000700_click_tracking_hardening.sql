...
create index if not exists idx_clicks_ip_created_at on public.clicks(ip_address, created_at desc);
create index if not exists idx_clicks_product_merchant_ip_created_at
  on public.clicks(product_id, merchant_id, ip_address, created_at desc);

create table if not exists public.click_tracking_rate_limits (
...
create table if not exists public.click_tracking_events (
...
create index if not exists idx_click_tracking_rate_limits_window
  on public.click_tracking_rate_limits(window_started_at, blocked_until);

create index if not exists idx_click_tracking_events_created
  on public.click_tracking_events(created_at desc);

create index if not exists idx_click_tracking_events_reason
  on public.click_tracking_events(reason, created_at desc);

alter table public.click_tracking_rate_limits enable row level security;
alter table public.click_tracking_events enable row level security;

drop policy if exists click_tracking_rate_limits_admin_read on public.click_tracking_rate_limits;
create policy click_tracking_rate_limits_admin_read on public.click_tracking_rate_limits
  for select
  using ((select public.is_admin(auth.uid())));

drop policy if exists click_tracking_rate_limits_admin_write on public.click_tracking_rate_limits;
create policy click_tracking_rate_limits_admin_write on public.click_tracking_rate_limits
  for all
  using ((select public.is_admin(auth.uid())))
  with check ((select public.is_admin(auth.uid())));

drop policy if exists click_tracking_events_admin_read on public.click_tracking_events;
create policy click_tracking_events_admin_read on public.click_tracking_events
  for select
  using ((select public.is_admin(auth.uid())));

drop policy if exists click_tracking_events_admin_write on public.click_tracking_events;
create policy click_tracking_events_admin_write on public.click_tracking_events
  for all
  using ((select public.is_admin(auth.uid())))
  with check ((select public.is_admin(auth.uid())));

drop trigger if exists trg_click_tracking_rate_limits_updated_at on public.click_tracking_rate_limits;
create trigger trg_click_tracking_rate_limits_updated_at
before update on public.click_tracking_rate_limits
...
create or replace function public.log_click_tracking_event(
  p_reason text,
  p_ip_address text,
...
as $$
  insert into public.click_tracking_events (
    reason,
    ip_address,
    product_id,
    merchant_id,
  ...
  values (
    left(coalesce(nullif(trim(p_reason), ''), 'unknown'), 100),
    left(coalesce(nullif(trim(p_ip_address), ''), 'unknown'), 120),
    p_product_id,
    p_merchant_id,
    p_offer_id,
    left(coalesce(nullif(trim(p_user_agent), ''), 'unknown'), 255),
...
declare
  ...
  v_dedupe_window interval := interval '45 seconds';
  v_max_requests integer := 25;
...
begin
  ...
  if v_duplicate_exists then
    perform public.log_click_tracking_event(
      ...
      p_offer_id => p_offer_id,
      p_user_agent => v_user_agent,
      p_metadata => jsonb_build_object('dedupeWindowSeconds', 45)
    );
...
grant execute on function public.log_click_tracking_event(text, text, uuid, uuid, uuid, text, jsonb) to anon, authenticated;
